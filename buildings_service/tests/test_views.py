...
class TestIndex(TestCase):
    def test_index_renders_correct_html(self):
        ...
        self.assertTemplateUsed(response, 'buildings_service/index.html')
...
class TestExplore(TestCase):
    ...
    def test_buildings_are_returned_in_response(self):
        ...
        response = self.client.get(reverse('explore'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'buildings_service/building_list.html')
...
class TestUpload(TestCase):
    def test_renders_form_correctly_in_a_GET_request(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'buildings_service/upload.html')
        # File in form
        self.assertContains(
            response,
